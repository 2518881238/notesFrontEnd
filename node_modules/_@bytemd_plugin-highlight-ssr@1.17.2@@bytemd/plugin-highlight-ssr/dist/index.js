...
function compileLanguage(language2) {
  function langRe(value, global) {
    return new RegExp(source(value), "m" + (language2.case_insensitive ? "i" : "") + (language2.unicodeRegex ? "u" : "") + (global ? "g" : ""));
  }
  class MultiRegex {
    constructor() {
      this.matchIndexes = {};
...
const HLJS = function(hljs) {
  ...
  function highlightElement(element2) {
    ...
    if (shouldNotHighlight(language2))
      return;
    fire("before:highlightElement", { el: element2, language: language2 });
    if (element2.children.length > 0) {
      ...
      if (options.throwUnescapedHTML) {
        const err = new HTMLInjectionError("One of your code blocks includes unescaped HTML.", element2.innerHTML);
        throw err;
      }
    }
    node = element2;
    const text = node.textContent;
    const result = language2 ? highlight2(text, { language: language2, ignoreIllegals: true }) : highlightAuto2(text);
...
function highlight(language2, value, options = {}) {
  ...
  if (!HighlightJS.getLanguage(language2)) {
  ...
  const result = HighlightJS.highlight(value, { language: language2, ignoreIllegals: true });
...
function registerLanguage(language2, syntax) {
  HighlightJS.registerLanguage(language2, syntax);
}
const registerAlias = function(language2, alias) {
  if (typeof language2 === "string") {
    HighlightJS.registerAliases(alias, { languageName: language2 });
  } else {
    let key;
    for (key in language2) {
      if (own$1.call(language2, key)) {
        HighlightJS.registerAliases(language2[key], { languageName: key });
      }
    }
  }
};
...
function highlightSsr({
  ...
  return {
    rehype: (processor) => processor.use(rehypeHighlight, { subset, ignoreMissing, ...rest })
  };
}
module.exports = highlightSsr;
